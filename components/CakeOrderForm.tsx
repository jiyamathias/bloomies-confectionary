...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="border border-[rgba(165,140,244,0.15)] rounded-2xl p-5 space-y-4 bg-white">
      <p className="font-cormorant text-[1.1rem] font-semibold text-[#433075] border-b border-[rgba(165,140,244,0.12)] pb-2">{title}</p>
      {children}
    </div>
  );
}
...
function OneLayerForm({ product, onClose }: { product: Product; onClose?: () => void }) {
  ...
  // Which fields are enabled for this specific cake
  // If customizationOptions is undefined (old data), show everything
  ...
  function buildWhatsApp() {
    ...
    const msg = `🌸 *Bloomies Order — One-Layer Cake*\n\n` +
      ...
      (has('message')  ? `✍️ Message on cake: ${message || 'None'}\n` : '') +
  ...
  function addToCart() {
    const summary = [
      has('flavour') ? flavour : '',
      has('icing')   ? (icing === 'whipped-cream' ? 'Whipped Cream' : 'Buttercream') : '',
      has('colour')  ? colour : '',
  ...
  return (
    <div className="space-y-4">
      {has('flavour') && (
        <Section title={`${step++}. Choose Your Flavour`}>
      ...
      {has('icing') && (
        <Section title={`${step++}. Type of Icing`}>
      ...
      {has('colour') && (
        <Section title={`${step++}. Cake Colour & Theme`}>
      ...
      {has('message') && (
        <Section title={`${step++}. Cake Message`}>
      ...
      {(has('occasion') || has('gender')) && (
        <Section title={`${step++}. Occasion & Recipient`}>
          <div className="grid grid-cols-2 gap-4">
            {has('occasion') && <Select label="Occasion" value={occasion} onChange={setOccasion} options={OCCASIONS} />}
            {has('gender')   && <Select label="Gender of receiver" value={gender} onChange={setGender} options={GENDERS} />}
          </div>
        </Section>
      )}

      {has('delivery') && (
        <Section title={`${step++}. Delivery Details`}>
      ...
      {/* Add-ons section — only shown if at least one add-on is enabled */}
      {(has('addon_card') || has('addon_chamdor') || has('addon_candles')) && (
        <Section title={`${step++}. ✨ Add-Ons`}>
      ...
      {has('notes') && (
        <Section title={`${step++}. 💬 Additional Notes`}>
      ...
      <div className="bg-[#433075] rounded-2xl p-5 space-y-2">
        ...
        {icingSurcharge > 0 && <div className="flex justify-between text-[0.85rem] text-white/70"><span>Whipped cream surcharge</span><span>+{fmt(icingSurcharge)}</span></div>}
        {has('addon_card')    && card    && <div className="flex justify-between text-[0.85rem] text-white/70"><span>Greeting card</span><span>+{fmt(3000)}</span></div>}
        {has('addon_chamdor') && chamdor && <div className="flex justify-between text-[0.85rem] text-white/70"><span>Chamdor wine</span><span>+{fmt(6000)}</span></div>}
        {has('addon_candles') && candles > 0 && <div className="flex justify-between text-[0.85rem] text-white/70"><span>Candles ×{candles}</span><span>+{fmt(candles * 500)}</span></div>}
        <div className="border-t border-white/15 pt-3 flex justify-between">
          ...
          <span className="font-cormorant text-[1.3rem] font-semibold text-[#E1D7F0]">{fmt(total)}</span>
        </div>
...
function BiggerCakeForm({ product, onClose }: { product: Product; onClose?: () => void }) {
  ...
  const addOnTotal =
    (has('addon_card')    && card    ? 3000 : 0) +
    (has('addon_chamdor') && chamdor ? 6000 : 0) +
    (has('addon_candles') ? candles * 500 : 0);
  ...
  function buildWhatsApp() {
    ...
    const msg = `🌸 *Bloomies Order — Custom Cake*\n\n` +
      ...
      (has('size_layers')     ? `📏 Size: ${inches} | Layers: ${layers}\n` : '') +
      (has('flavour_regular') || has('flavour_deluxe') ? `🍰 Flavours: ${flavours.join(', ') || 'Not specified'}\n` : '') +
      (has('colour')          ? `🎨 Colour preference: ${colour}${customColour ? ` (${customColour})` : ''}\n` : '') +
  ...
  return (
    <div className="space-y-4">
      {/* Design style info banner — set by admin, shown to customer as context */}
      {product.designStyle && (
        <div className="bg-[#F3F0FA] border border-[rgba(165,140,244,0.25)] rounded-xl px-4 py-3 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[0.7rem] text-[#6E6A8C]">This is the design for this cake. You can add your colour preference below.</p>
          </div>
        </div>
      )}

      {has('size_layers') && (
        <Section title={`${step++}. Size & Layers`}>
          <div className="grid grid-cols-2 gap-4">
      ...
      {(has('flavour_regular') || has('flavour_deluxe')) && (
        <Section title={`${step++}. Choose Flavours`}>
          {has('flavour_regular') && (
            <>
              <Label text="Regular Flavours" />
              <div className="flex flex-wrap gap-2 mb-3">
                {BIGGER_FLAVOURS_REG.map(f => <Chip key={f} label={f} on={flavours.includes(f)} onClick={() => toggleFlavour(f)} />)}
              </div>
            </>
          )}
          {has('flavour_deluxe') && (
            <>
              ...
              <div className="flex flex-wrap gap-2">
                {BIGGER_FLAVOURS_DLX.map(f => <Chip key={f} label={f} on={flavours.includes(f)} onClick={() => toggleFlavour(f)} />)}
              </div>
              <p className="text-[0.7rem] text-[#6E6A8C]/70 mt-2 italic">✨ Deluxe flavours may attract additional charges</p>
            </>
          )}
        </Section>
      )}
      ...
      {has('colour') && (
        <Section title={`${step++}. Cake Colour`}>
          <div className="flex flex-wrap gap-2 mb-3">
            {COLOURS.map(c => <Chip key={c} label={c} on={colour === c} onClick={() => setColour(c)} />)}
          </div>
          <Input label="Custom colour / theme (optional)" value={customColour} onChange={setCustomColour}
      ...
      {has('topper') && (
        <Section title={`${step++}. Cake Topper`}>
          <div className="flex flex-wrap gap-2">
